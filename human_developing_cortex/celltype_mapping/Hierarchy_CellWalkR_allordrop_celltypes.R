...
# for orignal or for experimenting drop cell types
...
dropcell = 'all_nEN' #'nEN_early2_nEN_late'
load(paste0('/pollard/data/projects/zhhu/cellwalk/bootstrap/cellwalk_integrate_all_3000_cor_drop_', dropcell, '_ss1_0.robj'))
...
results = removeTip(cellTypesM, allCellTypes, 'IPC-nEN1', weight = down, side = 'up')
cellTypesM = results[[1]]
allCellTypes = results[[2]]

results = removeTip(cellTypesM, allCellTypes, 'nEN-early2', weight = down, side = 'up')
cellTypesM = results[[1]]
allCellTypes = results[[2]]

results = removeTip(cellTypesM, allCellTypes, 'IPC-nEN2', weight = down, side = 'up')
cellTypesM = results[[1]]
allCellTypes = results[[2]]

results = removeTip(cellTypesM, allCellTypes, 'nEN-late', weight = down, side = 'up')
cellTypesM = results[[1]]
allCellTypes = results[[2]]
...
write.csv(as.matrix(aa), file = paste0("permute_Geshwind/cellwalk_integrate_all_3000_cor_drop_",dropcell ,"_wtree_info_rand1_",args[3],".csv"))
...
write.csv(as.matrix(aa2), file = paste0("permute_Geshwind/cellwalk_integrate_all_3000_cor_drop_", dropcell, "_wtree_info1_rand1_",args[3],".csv"))
