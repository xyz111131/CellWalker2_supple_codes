...
# varying number of markers per cell type and for experimenting combined EN cell types
# only permute Polioudakis labels to cells
...
if(is.na(percent)) percent = 1
down= 0.1
wtree= 1

print(paste(weight1, weight2, wtree, down, percent))

## recompute cell-label edges by varying number of markers ##
markers = read.csv("science_markers.csv", row.names=1)
...
markers = markers[abs(avg_diff)>1] # orignially 1 (0.5 for 200%), keep both positive and negative markers
# randomly selection different percentages of markers, around 500 markers per cluster originally
if(percent < 1)
{
  set.seed(123)
  markers = markers[sample(nrow(markers), round(percent*nrow(markers)))]
}
markers2 = data.table(markers2)
markers2 = markers2[abs(avg_log2FC)>0.5] # originally 0.5 (0.25 for 200%)
if(percent < 1)
{
  markers2 = markers2[sample(nrow(markers2), round(percent*nrow(markers2)))] # around 870 markers per cluster originally
}
...
# combine nodes
combine_celltypes = c('nEN-early2', 'nEN-late','IPC-nEN1', 'IPC-nEN2') 
ll = sapply(combine_celltypes, function(x) markers[cluster==x, ][['gene']]  )
llll = NULL
for(i in 1:(length(ll) -1))
{
  for(j in (i+1):length(ll))
  {
    llll = union(llll, intersect(ll[[i]], ll[[j]]))
  } 
}

m2comb = markers[(cluster %in% combine_celltypes) & gene %in% llll ]
m2comb[, cluster:=NULL]
m2comb = m2comb[, lapply(.SD, mean),  by = gene]
m2comb[, cluster:= 'nEN']

markers = markers[!(cluster %in% combine_celltypes) ]
markers = rbind(markers, m2comb) 
...
## remove tips from the tree
results = removeTip(cellTypesM, allCellTypes, 'U3', weight = down, side = 'up')
cellTypesM = results[[1]]
allCellTypes = results[[2]]

for(ct in combine_celltypes[1:3])
{
  results = removeTip(cellTypesM, allCellTypes, ct, weight = down, side = 'up')
  cellTypesM = results[[1]]
  allCellTypes = results[[2]]
}
allCellTypes[allCellTypes == combine_celltypes[4]] = 'nEN'
...
#write.csv(as.matrix(aa), file = paste0("permute_Geshwind/cellwalk_integrate_all_3000_cor_wtree_info_rand1_markers_",args[4], '_', args[3],".csv"))
write.csv(as.matrix(aa), file = paste0("permute_Geshwind/cellwalk_integrate_all_3000_cor_wtree_info_rand1_markers_combine_nEN_", args[3],".csv"))
...
#write.csv(as.matrix(aa2), file = paste0("permute_Geshwind/cellwalk_integrate_all_3000_cor_wtree_info1_rand1_markers_",args[4], '_', args[3],".csv"))
write.csv(as.matrix(aa2), file = paste0("permute_Geshwind/cellwalk_integrate_all_3000_cor_wtree_info1_rand1_markers_combine_nEN_", args[3],".csv"))
