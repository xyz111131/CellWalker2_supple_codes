...
# for perturbing cell-cell graph
load("seurat_merge_all.rdat")
set.seed(456)
noise = as.numeric(args[4]) # for the top 40 pcs, sd ranges from 6.67 to 1.48 
if(is.na(noise)) noise = 0
#graph = cortex@graphs$integrated_nn
#graph2 = graph %*% t(graph) #20 nearest neighbor
#graph2[graph2 < 3] = 0
#cellEdges = graph2
#cellEdges@x = graph2@x/(40-graph2@x) 

# add noise to the expression matrix
if(noise > 0)
{
  data.use = cortex@reductions$pca@cell.embeddings[,1:40]
  data.use = data.use + noise * sapply(1:ncol(data.use), function(i) rnorm(nrow(data.use)))
  knn_graph = FindNeighbors(data.use, k.param = 20) #distance.matrix = T, 
  cellEdges = knn_graph$snn # will set diag = 0 later
  stopifnot(all(colnames(cellEdges) == rownames(labelEdges)))
  print(cellEdges[1:5,1:5])
}
...
write.csv(as.matrix(aa), file = paste0("permute_Geshwind/cellwalk_integrate_all_3000_cor_noise_", noise ,"_wtree_info_rand1_",args[3],".csv"))
...
write.csv(as.matrix(aa2), file = paste0("permute_Geshwind/cellwalk_integrate_all_3000_cor_noise_", noise, "_wtree_info1_rand1_",args[3],".csv"))
