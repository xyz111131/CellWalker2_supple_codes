...
aa= aa[matrixStats::rowMaxs(as.matrix(aa)) > 3, ]
...
filterTFexp <- function(aa, with_internal = T, to.scale = T)
{
  # load gene expression
  pbmc1 = readRDS('human_Inh_SNARE_RNASeq_seurat.rds')
  ##symbols <- mapIds(org.Hs.eg.db, keys = toupper(rownames(aa)), keytype = "SYMBOL", column="ENSEMBL")
  all(toupper(rownames(aa)) %in% rownames(pbmc1@assays$RNA@data))
  ##ind = which(symbols %in% rownames(pbmc1@assays$RNA@data))
  ##aa = aa[ind, ]
  tf_exp = pbmc1@assays$RNA@data[toupper(rownames(aa)),]
  ...
  if(to.scale) tf_exp = t(scale(t(tf_exp)))
  tf_exp = reshape2::melt(as.matrix(tf_exp))
  # load cell type label
  tf_exp$cluster = pbmc1@meta.data[tf_exp$Var2, 'subclass']
  tf_exp$cluster = stringr::str_to_title(tf_exp$cluster)
  if(with_internal)
  {
    # add internal nodes of the tree
    tf_exp$cluster2[tf_exp$cluster %in% c('Sncg', 'Vip')] = 'Vip.Sncg.1'
    tf_exp$cluster2[tf_exp$cluster %in% c('Sst', 'Pvalb')] = 'Sst.Pvalb.1'
    tf_exp$cluster3[tf_exp$cluster %in% c('Sncg', 'Vip', 'Lamp5')] = 'Lamp5.Sncg.2'
  }
...
convert2plot <- function(aa, tf_exp_mean, ord = NULL, ord2 = NULL,  label = 'zscore', th = 3)
{
  ...
  if(is.null(ord))
  {
    ...
    ord = ord$order
    ord[1:5]  = ord[c(2:5,1)] # just to switch NFIA
  }
  if(is.null(ord2))
  {
...
nn = sapply(1:ncol(res[[1]]), function(i) {
  ...
  intersect(x, rownames(res[[2]])[res[[2]][,i] > 0.2]) # less strict
})
names(nn) = colnames(res[[1]])
save(nn, file = "human_sig_TF_per_celltype.rdat")

load('marmoset_sig_TF_per_celltype.rdat') # order the TFs as shared between human and marmoset goes first, then unique
...
tf_exp_mean = res[[2]]
ord = unique(unlist(nn))
rest = setdiff(unique(unlist(nn)), TF2)
ord = c('TCF4', 'ZEB1','SOX4', 'NFIX', 'NFIB','SP8',  'SP4', 'SP3', 'SOX2', 'SOX13', 'NFIA', 'KLF13','RORA', 'MEF2C', 'Esrrg','MEF2A', 'ESRRA', 'BACH1','NFE2L1', 
        'Npas2', 'MYC', 'VSX1',  'NR2C1', 
         'KLF6', "KLF5", "ZNF148", 'MAZ', 'SP9', 'Klf12','GLIS1','ELK4','HIF1A','NRF1', "TCFL5")
ord2 = c('Vip', 'Sncg', 'Vip.Sncg.1', 'Lamp5', 'Lamp5.Sncg.2', 'Pvalb', 'Sst.Pvalb.1', 'Sst.Chodl')
res2 = convert2plot(res[[1]][ord, ], tf_exp_mean[ord, ],ord = ord, ord2 = ord2,  th = 2.5)
aa2 = res2[[1]]
tf_exp_mean2 = res2[[2]]
##tf_exp_mean2[tf_exp_mean2$expression > 5, 'expression'] = 5 
col = res2[[3]]
pdf(paste0(prefix, 'Figure8B_', species, "_Inh_", level, "_TF_motif_signac_info1_single_rand_log_Jaccard_withTF_sel_expr_reorder2.pdf"),width=4, height = 9) #14
