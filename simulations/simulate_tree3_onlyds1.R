...
accuracy_all = foreach(ss = 1:nsim, .combine = 'rbind') %dopar%
{
  ...
  sim <- splatSimulate(nGenes = 1200, batchCells = c(ncell/3, (ncell*2)/3), batch.facLoc = 0.1, batch.facScale = 0.1, #nGenes = 1000
  ...
  sim.groups1 <- splatSimulate(group.prob = c(0.5, 0.5), de.prob = c(0.5,0.5), nGenes = 100, lib.loc = log(1e5/5), de.facScale = 0.2, #nGenes = 200, de.facScale = default
                               method = "groups", verbose = FALSE, batchCells = c(ncell/3, (ncell*2)/3),
                               dropout.mid = c(1, 2), dropout.shape = c(-1, -0.5),dropout.type = 'batch') #dropout.mid = c(0, 2),
  
  # simulate 100 DE genes between (1,2)
  sim.groups2 <- splatSimulate(group.prob = rep(0.25, 4), de.prob = c(0.8,0.8, 0, 0), 
                               nGenes = 150,  de.facLoc = 0.1, de.facScale = 0.2, lib.loc = log(1e5/5), #nGenes = 200, dropout.mid = c(0, 2),
                               method = "groups", verbose = FALSE, batchCells = c(ncell/3, (ncell*2)/3),
