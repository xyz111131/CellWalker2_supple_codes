...
## dataset 2 cell type 5 has features with cell type 2 and 4
library(scater)
...
prefix = "~/Dropbox (Gladstone)/cell_hierarchy/cellwalk/simulation_results_new/simulation_results5/"

#accuracy_all = foreach(ss = 1:nsim, .combine = 'rbind') %dopar%
#{
  print(paste('simulation', ss))
  # simulate 900 genes without DE, batch loc 0.1, scale 0.1
  ...
  # simulate 100 DE genes between (1,2) and (3,4,5), batch for different cell types and data sets, loc 0.1, scale 0.4
  sim.groups1 <- splatSimulate(group.prob = c(0.4, 0.6), de.prob = c(1,1), nGenes = 100, lib.loc = log(1e5/9),
  ...
  # simulate 200 DE genes between (1,(2,5))
  sim.groups2 <- splatSimulate(group.prob = c(0.2, 0.4, rep(0.2, 2)), de.prob = c(1,1, 0, 0),
                               nGenes = 300,  de.facLoc = 0.1, de.facScale = 0.4, lib.loc = log(1e5/3),
                               method = "groups", verbose = FALSE, batchCells = rep(ncell/2,2),batch.facLoc = 0.01, batch.facScale = 0.1,
                               dropout.mid = c(0, 0), dropout.shape = c(-1, -1), dropout.type = 'batch') #0.1 0.2


  # simulate 100 DE genes between (3,(4,5))
  sim.groups3 <- splatSimulate(group.prob = c(rep(0.2, 3), 0.4), de.prob = c(0,0, 1, 1),
                               nGenes = 100,  de.facLoc = 0.1, de.facScale = 0.4, lib.loc = log(1e5/9),
                               method = "groups", verbose = FALSE, batchCells = rep(ncell/2,2),batch.facLoc = 0.01, batch.facScale = 0.1,
                               dropout.mid = c(0, 0), dropout.shape = c(-1, -1), dropout.type = 'batch') #0.1 0.2
  # # simulate 100 DE genes between (4,5)
  # sim.groups4 <- splatSimulate(group.prob = rep(0.2, 5), de.prob = c(0,0, 0, 1, 1),
  #                              nGenes = 100,  de.facLoc = 0.1, de.facScale = 0.4, lib.loc = log(1e5/9),
  #                              method = "groups", verbose = FALSE, batchCells = rep(ncell/2,2),batch.facLoc = 0.01, batch.facScale = 0.1,
  #                              dropout.mid = c(0, 0), dropout.shape = c(-1, -1), dropout.type = 'batch') #0.1 0.2
  ...
  #                              dropout.mid = c(0, 0), dropout.shape = c(-1, -1),dropout.type = 'batch')
  # 
  # # simulate 100 DE genes between (1,2,5) and (3, 4)
  # sim.groups4 <- splatSimulate(group.prob = c(0.6, 0.4), de.prob = c(1,1), 
  #                              nGenes = 200,  lib.loc = log(1e5/5),
  #                              method = "groups", verbose = FALSE, batchCells = rep(ncell/2,2),batch.facLoc = 0.01, batch.facScale = 0.1,
  #                              dropout.mid = c(0, 0), dropout.shape = c(-1, -1), dropout.type = 'batch') #0.1 0.2
  # 
  # # simulate 100 DE genes between (1,(2,5))
  # sim.groups2 <- splatSimulate(group.prob = c(0.2, 0.4, rep(0.2, 2)), de.prob = c(1,1, 0, 0),
  #                              nGenes = 200,  lib.loc = log(1e5/5),
  #                              method = "groups", verbose = FALSE, batchCells = rep(ncell/2,2),batch.facLoc = 0.01, batch.facScale = 0.1,
  #                              dropout.mid = c(0, 0), dropout.shape = c(-1, -1), dropout.type = 'batch') #0.1 0.2
  ...
  meta.data = data.frame('Cell' = paste0('Cell', 1: ncell), 'Batch' = paste0('Batch', rep(1:2, each  = ncell/2)), 
                         'Group' = rep(paste0('Group', rep(1:nclus, each  = ncell/2/nclus)),2)) 
  rownames(meta.data) = meta.data$Cell
  # switch cells
  ind = which(colData(sim.groups2)$Batch == 'Batch1' & colData(sim.groups2)$Group == 'Group2')
  ind2 = ind[-1:-(length(ind)/2)]
  ct1 = sim.groups2@assays@data$counts[, 1:(ncell/2)]
  ct1 = cbind(ct1[, -ind2], ct1[, ind2])
  
  ct2 = sim.groups2@assays@data$counts[, -1:-(ncell/2)]
  ct2 = cbind(ct2[, -ind2], ct2[, ind2])
  sim.groups2@assays@data$counts = cbind(ct1, ct2)
  
  # ind = which(colData(sim.groups4)$Batch == 'Batch1' & colData(sim.groups4)$Group == 'Group1')
  # ind2 = ind[-1:-(length(ind)*2/3)]
  # ct1 = sim.groups4@assays@data$counts[, 1:(ncell/2)]
  # ct1 = cbind(ct1[, -ind2], ct1[, ind2])
  # 
  # ct2 = sim.groups4@assays@data$counts[, -1:-(ncell/2)]
  # ct2 = cbind(ct2[, -ind2], ct2[, ind2])
  # sim.groups4@assays@data$counts = cbind(ct1, ct2)
  
  counts = rbind(sim@assays@data$counts, sim.groups1@assays@data$counts, 
                 sim.groups2@assays@data$counts, sim.groups3@assays@data$counts) #, sim.groups4@assays@data$counts) #TrueCounts
  ...
  counts1 = counts[, meta.data1$Cell]
  counts2 = counts[, meta.data2$Cell]
...
#  write.csv(pbmc.combined@meta.data, file = "../simulation_results_new/simulation_results4/simulation4-0_merge_metadata.csv")
  if(output_data)
  {
    write.csv(pbmc.combined@assays$RNA@counts, file = paste0(prefix, "simulation_data2/simulation5_", ss, "_merge_rawcount.csv"))
    write.csv(pbmc.combined@assays$RNA@scale.data, file = paste0(prefix, "simulation_data2/simulation5_", ss, "_merge_scaled.csv"))
    write.csv(pbmc.combined@meta.data, file = paste0(prefix, "simulation_data2/simulation5_", ss, "_merge_metadata.csv"))
  }
  ...
  if(output_data)
  {
    ...
    write.csv(cortex@assays$integrated@scale.data, file = paste0(prefix, "simulation_data2/simulation5_", ss, "_integrated_scaled.csv"))
    write.csv(cortex@meta.data, file = paste0(prefix, "simulation_data2/simulation5_", ss, "_integrated_metadata.csv"))
    quit()
  }
...
save(seurat_confusion, cellwalker_confusion, influence_score, Zscore, cellwalker2_confusion, file = paste0(prefix, '5_1_', ss, '.rdat'))
