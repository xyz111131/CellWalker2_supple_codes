...
#{
  ...
  confusion = confusion/rowSums(confusion)
  ...
  Idents(pbmc2) = 'predicted.id'
  markers_seurat = FindAllMarkers(pbmc2)# get DEG
  markers_seurat = data.table(markers_seurat)
  ...
  # get markers overlap
  markers_seurat = markers_seurat[abs(avg_log2FC)>0.5 & p_val_adj < 0.05]
  print(paste("number of markers_seurat:", markers_seurat[,.N, by = cluster]))
  mean_overlap = 0
  for(cl in unique(markers2$cluster))
  {
    g1 = unlist(markers2[cluster == cl, 'gene'])
    g2 = unlist(markers_seurat[cluster == cl, 'gene'])
    mean_overlap = mean_overlap + length(intersect(g1, g2))/length(union(g1,g2))
  }
  markers_overlap_seurat = mean_overlap/4
  ...
  # aa = cellWalk$infMat[nclus2, nclus]  #gesch -> UCSC
  ...
  pbmc2 <- AddMetaData(pbmc2, metadata = data.frame('cellwalk' = cellLabel))
  # get markers overlap
  Idents(pbmc2) = 'cellwalk'
  markers_cellwalk = FindAllMarkers(pbmc2)
  markers_cellwalk = data.table(markers_cellwalk)
  markers_cellwalk = markers_cellwalk[abs(avg_log2FC)>0.5 & p_val_adj < 0.05]
  print(paste("number of markers_cellwalk:", markers_cellwalk[,.N, by = cluster]))
  mean_overlap = 0
  for(cl in unique(markers2$cluster))
  {
    g1 = unlist(markers2[cluster == cl, 'gene'])
    g2 = unlist(markers_cellwalk[cluster == cl, 'gene'])
    mean_overlap = mean_overlap + length(intersect(g1, g2))/length(union(g1,g2))
  }
  markers_overlap_cellwalk = mean_overlap/4
  ...
  #                       weight1 = weight1, weight2 = weight2, nround = 20)
  # # map with Z-score
  # Zscore = results[[2]]
  # Zscore[Zscore < 0] = 0
  ...
  cellLabel =  sapply(result[[1]]$cellLabels[Cells(pbmc2)], function(x) strsplit(x, '_')[[1]][1]) 
  aa_tr = xtabs(~true_cluster +cellLabel)
  aa_tr = aa_tr/rowSums(aa_tr)
  accuracy3 = sum(diag(aa_tr))/sum(aa_tr)
  print(paste('number of NA: ', sum(is.na(cellLabel))))
  
  pbmc2 <- AddMetaData(pbmc2, metadata = data.frame('cellwalk_tr' = cellLabel)) #sapply(cellLabel, function(x) paste0('Group', x))))
  # get markers overlap
  Idents(pbmc2) = 'cellwalk_tr'
  markers_cellwalk = FindAllMarkers(pbmc2)
  markers_cellwalk = data.table(markers_cellwalk)
  markers_cellwalk = markers_cellwalk[abs(avg_log2FC)>0.5 & p_val_adj < 0.05]
  print(paste("number of markers_cellwalk:", markers_cellwalk[,.N, by = cluster]))
  ...
  markers_overlap_cellwalk_tr = mean_overlap/4
  
  
  print("Seurat:")
  print(accuracy)
  print(map_accuracy)
  print(markers_overlap_seurat)
  
  print("cellWalker:")
  print(accuracy2)
  print(accuracy3)
  print(markers_overlap_cellwalk)
  print(markers_overlap_cellwalk_tr)

save(confusion, aa, aa_tr, markers_overlap_cellwalk, markers_overlap_cellwalk_tr, markers_overlap_seurat,
     file = paste0(prefix, '3_both_', subn,'_', ss, '.rdat'))
...
# boxplot(accuracy[, 1:3], ylab = "cell annotation accuracy")
