...
## dataset 2 has a new cell type slightly different to the cell type in dataset1
library(scater)
...
#{
  ...
  sim <- splatSimulate(nGenes = 900, batchCells = rep(ncell/2,2), batch.facLoc = 0.01, batch.facScale = 0.1,
                       method = "single", verbose = FALSE, dropout.mid = c(0, 0), dropout.shape = c(-1, -1),
                       dropout.type = 'batch')
  
  # simulate 200 DE genes between (1,2) and (3,4,5), batch for different cell types and data sets, loc 0.1, scale 0.4
  ...
  sim.groups2 <- splatSimulate(group.prob = rep(0.2, 5), de.prob = c(1,1, 0, 0, 0), 
                               nGenes = 300,  de.facLoc = 0.15, de.facScale = 0.2, lib.loc = log(1e5/5),
                               method = "groups", verbose = FALSE, batchCells = rep(ncell/2,2),batch.facLoc = 0.01, batch.facScale = 0.1,
  ...
  sim.groups3 <- splatSimulate(group.prob = c(rep(0.2, 3), 0.4), de.prob = c(0,0, 1, 1), 
                               nGenes = 200,  de.facLoc = 0.15, de.facScale = 0.2, lib.loc = log(1e5/5),
                               method = "groups", verbose = FALSE, batchCells = rep(ncell/2,2),batch.facLoc = 0.01, batch.facScale = 0.1,
                               dropout.mid = c(0, 0), dropout.shape = c(-1, -1), dropout.type = 'batch') #0.1 0.2
  # simulate 100 DE genes between (4,5)
  sim.groups4 <- splatSimulate(group.prob = rep(0.2, 5), de.prob = c(0,0, 0, 1, 1), 
                               nGenes = 100,  de.facLoc = 0.1, de.facScale = 0.2, lib.loc = log(1e5/5),
                               method = "groups", verbose = FALSE, batchCells = rep(ncell/2,2),batch.facLoc = 0.01, batch.facScale = 0.1,
                               dropout.mid = c(0, 0), dropout.shape = c(-1, -1), dropout.type = 'batch') #0.1 0.2
 ...
 # d2 = cbind(rowData(sim.groups3), "DEFacGroup5" = rowData(sim.groups3)[, "DEFacGroup4"])
 # genes = rbind(d1, rowData(sim.groups2), d2, rowData(sim.groups4))
 # genes$Gene = paste0('Gene', 901:1700)
 # rownames(genes) = genes$Gene
  ...
  #write.csv(pbmc.combined@assays$RNA@counts, file = "../simulation_results_new/simulation_results4/simulation4_merge_rawcount.csv")
  #write.csv(pbmc.combined@meta.data, file = "../simulation_results_new/simulation_results4/simulation4_merge_metadata.csv")
  if(output_data)
  {
    write.csv(pbmc.combined@assays$RNA@counts, file = paste0(prefix, "simulation_data2/simulation4_", ss, "_merge_rawcount.csv"))
    write.csv(pbmc.combined@assays$RNA@scale.data, file = paste0(prefix, "simulation_data2/simulation4_", ss, "_merge_scaled.csv"))
    write.csv(pbmc.combined@meta.data, file = paste0(prefix, "simulation_data2/simulation4_", ss, "_merge_metadata.csv"))
  }
  ...
  if(output_data)
  {
    ...
    write.csv(cortex@assays$integrated@scale.data, file = paste0(prefix, "simulation_data2/simulation4_", ss, "_integrated_scaled.csv"))
    write.csv(cortex@meta.data, file = paste0(prefix, "simulation_data2/simulation4_", ss, "_integrated_metadata.csv"))
    quit()
  }
...
save(seurat_confusion, cellwalker_confusion, influence_score, Zscore, cellwalker2_confusion, file = paste0(prefix, '4_', ss, '.rdat'))
