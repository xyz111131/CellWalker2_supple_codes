...
## dataset 2 has a broad cell type
library(scater)
...
#{
  ...
  sim.groups3 <- splatSimulate(group.prob = rep(0.2, 5), de.prob = c(0,0, 1, 1, 0), 
  ...
  meta.data2 = meta.data[meta.data$Batch == 'Batch2' & meta.data$Group != 'Group4' & meta.data$Group != 'Group3',]
  ...
  #write.csv(pbmc.combined@assays$RNA@counts, file = "../simulation_results_new/simulation_results4/simulation4-1_merge_rawcount.csv")
  if(output_data)
  {
    write.csv(pbmc.combined@assays$RNA@counts, file = paste0(prefix, "simulation_data2/simulation4-1_", ss, "_merge_rawcount.csv"))
    write.csv(pbmc.combined@assays$RNA@scale.data, file = paste0(prefix, "simulation_data2/simulation4-1_", ss, "_merge_scaled.csv"))
    write.csv(pbmc.combined@meta.data, file = paste0(prefix, "simulation_data2/simulation4-1_", ss, "_merge_metadata.csv"))
  }
  ...
  if(output_data)
  {
    ...
    write.csv(cortex@assays$integrated@scale.data, file = paste0(prefix, "simulation_data2/simulation4-1_", ss, "_integrated_scaled.csv"))
    write.csv(cortex@meta.data, file = paste0(prefix, "simulation_data2/simulation4-1_", ss, "_integrated_metadata.csv"))
    quit()
  }
  ...
  #print(round(confusion[-c(3:4),]/rowSums(confusion[-c(3:4),]),2))
  ...
  #print(round(aa[c(1,2,5),]/rowSums(aa[c(1,2,5),]),2))
  
  ## run CellWalker2 with tree
  tr01 <- ape::read.tree(text='((1, 2), (3, 4));')
  tr02 <- ape::read.tree(text='((1, 2), 5);')
...
# #check markers
# markers2$id = as.numeric(sub("Gene", "", markers2$gene))
# hist(markers2$id[markers2$cluster == 1])
# hist(markers2$id[markers2$cluster == 2])
# hist(markers$id[markers$cluster == 3])
# hist(markers$id[markers$cluster == 4])
# hist(markers2$id[markers2$cluster == 5])
  
#save(seurat_confusion, cellwalker_confusion, influence_score, Zscore, cellwalker2_confusion, file = paste0(prefix, '4-1_', ss, '.rdat'))
