...
meta['id'] = [ x[4:] for x in meta.Cell]
meta.id = meta.id.astype('int')

# subsample cells from group 3, remove group 3 in DS2
meta = meta[~((meta.Group == 'Group3') & (meta.Batch == 'Batch2'))]
meta = meta[~((meta.Group == 'Group4') & (meta.id > 1200 + int(sys.argv[2]))) ]

dat = dat.loc[:, meta.index]

meta.loc[:,['Group','Batch']].value_counts()
